...
追加指示：**継続状態を確認した直後、次の制作項目を選ぶ前にも必ず期限内達成を評価する。** 残作業・同種作業の実測速度・手戻り・実画面と実機と外部比較の検証待ちから「見込みあり」「現行方法では届かない」「根拠不足」を区別する。届かないと判断したら、やり方や前提が間違っていることを疑い、代案を実装・比較する。「疑った」という記録だけで同じ小機能追加を繰り返さない。実行中の別制作がある場合は読み取り評価を行い、重複起動や並行書込みをせず、安全な節目で判断と方法変更の結果を引き継ぐ。

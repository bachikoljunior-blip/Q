...
const gripX=.095,pull=.36,halfString=.59;
...
export function createBowContact(actor){
  ...
  return {bow,closedPad,restElbows:[new Vector3(),new Vector3()],anchorTurn:new Quaternion().setFromAxisAngle(new Vector3(0,1,0),-.5),upper:actor.g.getObjectByName('string-upper'),lower:actor.g.getObjectByName('string-lower'),
...
export function supportBow(actor,state,motion,solveArm){
  const b=actor.archery;if(!b||!motion.combat)return;
  ...
  b.grip.copy(b.origin).addScaledVector(b.forward,gripX+pull);
  ...
  const nockX=-pull*draw;
  b.upper.rotation.z=-Math.atan2(pull*draw,halfString);b.lower.rotation.z=Math.atan2(pull*draw,halfString);
  b.upper.scale.y=b.lower.scale.y=Math.hypot(halfString,pull*draw)/halfString;
  b.nock.set(nockX,0,0).applyMatrix4(b.bow.matrixWorld);

...
function projectileThreat(game,arrow,deferCover=false){
  ...
  if(!deferCover&&game.projectileContact(arrow,horizon).target!==player)return null;
  const t=fraction*horizon;
...
export function combatPresentation(game){
  ...
  for(const candidate of candidates){
    ...
    if(trajectory&&game.projectileContact(trajectory,horizon).target!==game.player)continue;

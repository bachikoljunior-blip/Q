...
export class Game {
  constructor(save=null) {
    ...
    this.time=0;this.day=.16;this.events=[];this.lit=['haven'];this.discovered=['haven'];this.talked=false;this.bossDefeated=false;this.ending=null;this.checkpoint='haven';this.relic=false;this.relicDelivered=false;this.assist=false;this.locked=null;this.pendingAction=null;
  ...
  attack() {
    ...
    const available=this.enemies.filter(e=>!e.dead&&!(e.type==='boss'&&this.lit.length<4)&&distance(p,e)<5&&this.canReach(p,e));
  ...
  tick(dt,input={}) {
    ...
    if(p.attack>0&&!this.attackHit&&p.attack<.27){this.attackHit=true;for(const e of this.enemies) {if(e.dead||distance(e,p)>(e.type==='boss'?4.9:3.5)||!this.canReach(p,e))continue;const angle=Math.atan2(e.x-p.x,e.z-p.z);if(Math.abs(angleDelta(angle,p.angle))<1.45)this.hurtEnemy(e,this.damageAmount()*(1+p.combo*.23));}}
  ...
  nearestInteract(){
    ...
    if(distance(p,KEEPER)<4.7)candidates.push(KEEPER);
  ...
  interact(target=this.nearestInteract()) {
    ...
    const canonical=target.id===KEEPER.id?KEEPER:PLACES.find(s=>s.id===target.id)||this.pickups.find(l=>l.id===target.id&&!l.taken);
    if(!canonical||distance(p,canonical)>=(canonical.type==='npc'?4.7:['herb','chest','relic'].includes(canonical.type)?3.2:5.5)||!this.canReach(p,canonical))return false;
    target=canonical;
    if(target.type==='npc'){this.talked=true;this.emit('dialogue');this.emit('save');return true;}
    ...
    this.checkpoint=target.id;this.clearTransient();p.hp=p.maxHp;p.stamina=100;p.energy=100;p.potions=Math.max(3,p.potions);p.skillCooldown=0;this.notify('灯火で休息 — 生命力と霊薬が回復');this.emit('rest');this.emit('save');
  }
  ...
  fastTravel(id){
    ...
    const p=this.player;this.clearTransient();p.x=place.x;p.z=place.z+5;moveCircle(p,0,0,this.obstacles);p.y=heightAt(p.x,p.z);this.checkpoint=id;this.emit('travel',{place});this.emit('save');return true;
  }
  respawn(){const p=this.player,s=PLACES.find(s=>s.id===this.checkpoint)||PLACES[0];this.clearTransient();p.x=s.x;p.z=s.z+7;moveCircle(p,0,0,this.obstacles);p.y=heightAt(p.x,p.z);p.hp=p.maxHp;p.stamina=100;p.energy=100;p.potions=Math.max(3,p.potions);p.dead=false;p.invulnerable=2;p.skillCooldown=0;for(const e of this.enemies){if(!e.dead){e.x=e.homeX;e.z=e.homeZ;e.y=heightAt(e.x,e.z);e.hp=e.maxHp;e.poise=180;e.state='idle';e.timer=0;e.cooldown=1;e.attackCount=0;e.radial=false;e.hit=false;e.avoid=null;e.route=null;}}this.emit('save');}
  ...
  restore(s){
    ...
    for(const l of this.pickups)l.taken=Array.isArray(s.taken)&&s.taken.includes(l.id);for(const e of this.enemies){e.dead=(Array.isArray(s.defeated)&&s.defeated.includes(e.id))||(e.type==='boss'&&this.bossDefeated);if(e.dead)e.hp=0;}moveCircle(p,0,0,this.obstacles);p.y=heightAt(p.x,p.z);if(s.dead===true)this.respawn();return true;
  }
}

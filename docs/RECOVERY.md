...
現行方法では期限内達成ができないと判断したら、やり方や前提が間違っていることを疑う。優先順位、素材調達、実装単位、道具、検証工程の代案を選び、次に制作を行う際に実装・比較して、品質への効果と所要時間を別々に確認する。記録だけで同じ作業を繰り返さず、目標品質・完成期限を下げない。制作側の安全な節目で、判断だけでなく採用した方法とその実結果を残す。

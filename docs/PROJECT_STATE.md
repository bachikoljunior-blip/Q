...
- 前の方式は会話を開くまで相談の存在が分からず、画面と3Dで話者を結ぶ正規IDを持たず、既読発話を読み返せなかった。今回の状態検査では両場面で案内、発話ごとの一意な話者、保存後の一致、既読範囲、旧保存への切替時の非表示が成功。これは情報を提示する処理の比較であり、初見で気づけることや見やすさの比較ではない。

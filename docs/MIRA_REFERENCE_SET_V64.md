...
## 2026-09-16 15:35 UTC の固定範囲

直前の PR59 保存は source `43ce192048acd66223da11e3f3bf5383291d9416` / tree `6c9d64bb11d47bacc9837bc2a9f71f34d075f6a1`。required CI `35113609273` success、native取得と全tree一致を確認済み。以下はその後の資料保存で、新runtime差分ではない。

[全人物の画像対応](evidence/mira-reference-set-v64/WHOLE_REFERENCE_SELECTION.json)は頭41型82配置（旧40/80にF21一型二配置を追加）、衣服40/96、手10/56、靴11/34、杖18/19の計120型287配置。73枚の選択原PNGをrepo内の実ファイルとhashへ対応させた。型と配置の重複0、参照欠損0は資料の存在と選択の検査に限る。全人物の表面を隙間なく組めること、原画への見た目一致、動作や端末品質の証明ではない。番号は部位名を含め、頭・服・杖の同名IDを混同しない。

衣服の固定資料は40型96配置、37 raw PNGを含む103ファイル。[独立レビュー](evidence/mira-reference-set-v64/cloth-reference-review/REVIEW.md)は30選択画像・旧脚面4画像・完成像等6画像を実見し、閉じた筒になっていた脚面を一枚の開放面へ変えた方法Bを確認した。股布TR03の四辺の接合先、手首帯F03の巻き順、裾・靴の重なりは数値設計待ち。方法Bの青灰色は形状用で、完成像の素材色として採用しない。S06の一入力path記録が不完全な点も作者原記録に保持する。

頭部F21、F17側面、A01a耳輪の4生成callは終端。耳前の小面、単一の唇稜、耳輪裏の開U面を示す。A01a断面の不正確な視点表記は座標根拠から除外する。手靴の爪床/別爪板、連続した薄底/別踵、空の留め座、開いた甲面、手首袖の所有範囲は局所補完と独立追補で整理した。これらは数値共有辺と実メッシュ接触の合格ではない。

次は各作者が具体的な共有曲線・寸法・配置・髪根・骨weightを一案に選び、全身境界と整合させる。画像を実測値とせずauthor設計を明記する。実骨格から旧靴底包絡20 mmが確認でき、未採用の22 mm例ではなく底8 mm＋踵12 mmを設計する。旧手持ち杖が本人左なのに完成像は右である点も登録で訂正対象として保持する。全資料と組立対応が整う前の新しいメッシュ制作は0のまま。

生成・修正・設計の手戻りを含む現実の経過から、9月20日の全画面PS4品質達成を肯定する根拠はまだない。今回の単一型・開放面プロンプトという代案は資料の局所矛盾を減らしたが、造形速度や完成モデル品質への効果は未実証。正式GL無効、実材質/動作画面・実機性能・聴感・指定比較の未取得を維持する。mainは547676fd…、PR39 open/unmerged、Site24は2bee64cf…のまま。PRへの資料保存をゲーム更新と扱わない。

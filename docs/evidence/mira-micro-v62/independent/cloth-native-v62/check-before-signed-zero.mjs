...
for(const part of PARTS){
 const m=buildPart(part),record=saved.find(x=>x.id===m.id);assert.deepEqual(m,record,'fresh source must equal frozen mesh');
 ...
 rows.push({id:m.id,vertices:p.length,triangles:m.indices.length,boundaryEdges:boundary.length,boundaryLoops:1,nonManifold,sameDirectionEdges:sameDirection,minAreaM2:minArea,minFaceVertexNormalDot:minNormalDot,normalUnitError,weightSumError:weightError,freshMeshEqualsFrozen:true,objReadbackMaxErrorM:objMaxError,neighboursWelded:false,thicknessM:m.thicknessMetres,death045:{minWorldY:Math.min(...posed.map(p=>p.y)),minAreaM2:posedMinArea,minFaceVertexNormalDot:posedMinNormalDot,maxPositionDifferenceFromRecordedPoseM:poseReadbackError,finite:posed.every(p=>p.toArray().every(Number.isFinite))}});
}
const result={at:new Date().toISOString(),boundary:'One native source pass and one already-authored actual actor death .45 s pose. No rendering, new pose set, body or seam collision guarantee.',sourceSha256:sha('review/cloth-micro-v62/surfaces.mjs'),actorSha256:sha('src/actor-models.js'),rows,remainingRuntimeBlocker:'P01 chest-only weighting penetrates floor; free cloth/support and all adjacent seam contracts are unimplemented.'};

...
function decode(text,length){
  ...
  for(let i=0;i<text.length;i+=5){let n=0;for(let j=0;j<5;j++){const d=digits[text.charCodeAt(i+j)];if(d===undefined||d<0)throw Error('digit');n=n*85+d;}if(n>4294967295)throw Error('overflow');bytes[out]=n>>>24;if(out+1<length)bytes[out+1]=n>>>16;if(out+2<length)bytes[out+2]=n>>>8;if(out+3<length)bytes[out+3]=n;out+=4;}
  return bytes;
}
...
await writeFile(new URL('./comparison-fast.json',import.meta.url),JSON.stringify(summary,null,2)+'\n');console.log(JSON.stringify({...summary,rows:undefined}));

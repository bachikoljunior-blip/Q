...
function animateCape(actor,motion){
  ...
  if(motion.state==='death'){
    ...
    for(let column=0;column<cloth.columns;column++){
      const root=cloth.targets[column],direction=cloth.delta.subVectors(cloth.targets[column+cloth.columns],root);
      ...
      for(let i=column+cloth.columns;i<pos.count;i+=cloth.columns){
        ...
        let along=travel*Math.cos(angle),y=root.y-travel*Math.sin(angle);
        if(travel>straight&&radius>1e-8){
          const bend=Math.min(angle,(travel-straight)/radius);
        ...
        point.copy(root).addScaledVector(cloth.forward,along);point.x+=cloth.forward.z*side;point.z-=cloth.forward.x*side;point.y=Math.max(y,floor);
      }
    }

...
function animateCape(actor,motion){
  ...
  if(motion.state==='death'){
    ...
    if(!cloth){
      const points=Array.from({length:pos.count},()=>new Vector3()),links=[],seen=new Set(),indices=actor.cape.geometry.index.array;
      ...
      cloth=actor.cape.userData.contact={points,links,inverse:new Matrix4(),scale:new Vector3(),origin:new Vector3(),delta:new Vector3()};
    }

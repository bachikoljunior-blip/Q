...
function animateCape(actor,motion){
  ...
  if(motion.state==='death'){
    ...
    // Continue each fabric meridian along the floor while preserving its length.
    // A vertical clamp alone compresses whole rows and makes the solver buckle.
    for(let i=cloth.columns;i<pos.count;i++){
      const previous=i-cloth.columns,point=cloth.points[i],anchor=cloth.points[previous],direction=cloth.delta.subVectors(cloth.targets[i],cloth.targets[previous]);
      const length=Math.hypot(base[i*3]-base[previous*3],base[i*3+1]-base[previous*3+1],base[i*3+2]-base[previous*3+2])*scale;
      point.copy(anchor).addScaledVector(direction.normalize(),length);
      const sample=actor.groundHeight?.(point.x,point.z),floor=(Number.isFinite(sample)?sample:cloth.origin.y)+clearance;
      if(point.y<floor){
        direction.addScaledVector(cloth.forward,Math.max(0,.25-direction.dot(cloth.forward)));
        const dy=clamp(floor-anchor.y,-length,length),horizontal=Math.hypot(direction.x,direction.z),reach=Math.sqrt(Math.max(0,length*length-dy*dy));
        if(horizontal>1e-8){point.x=anchor.x+direction.x/horizontal*reach;point.z=anchor.z+direction.z/horizontal*reach;}point.y=anchor.y+dy;
      }
    }
    ...
    for(let i=0;i<pos.count;i++){const point=cloth.points[i].applyMatrix4(cloth.inverse);pos.setXYZ(i,point.x,point.y,point.z);}

...
function supportFallenBody(actor,motion){
  if(motion.state!=='death'||actor.type==='wolf')return;
  let support=actor.bodySupport;
  if(!support){
    // Source-surface extrema are selected once. Runtime contact evaluates this
    // small bone-local support cloud, rather than every rendered vertex.
    const groups=new Map(),point=new Vector3(),directions=[];
    for(const x of [-1,0,1])for(const y of [-1,0,1])for(const z of [-1,0,1])if(Math.abs(x)+Math.abs(y)+Math.abs(z)===1||x*y*z!==0)directions.push(new Vector3(x,y,z).normalize());
    ...
    support=actor.bodySupport={samples,point:new Vector3(),origin:new Vector3(),scale:new Vector3(),normal:new Vector3(),up:new Vector3(0,1,0),rotation:new Quaternion(),inverse:new Matrix4()};
  }
  ...
  support.rotation.setFromUnitVectors(support.up,support.normal);support.rotation.slerp(new Quaternion(),1-fall);actor.pelvis.quaternion.premultiply(support.rotation);

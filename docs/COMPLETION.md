...
最新直接指定は「PS4相当の品質達成 これ達成するまで絶対止めんな」、対象は画面全てとタイトル映像・豊かな音。完成0/7・10作品比較0/10は維持。地形/川/樹木/動作/武器VFX/実天空の検証済み制作がPR39〜41へ保存され、頭部と実楽器を結合中だが、PR39の通常merge未完了によりmain/Siteはまだv23配信時点。詳細と次の有限単位は最新PROJECT_STATE先頭を使う。実画面・実機・実聴・外部比較は未確認で、部分制作/試験数を完成受入へ加算しない。

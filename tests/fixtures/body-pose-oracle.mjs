...
export {createDetailedActor};
const oldURL=new URL('../../src/actor-models.js?body-before-v37',import.meta.url).href;
const source=readFileSync(new URL('../../docs/evidence/body-pose-v37/source-before.js',import.meta.url),'utf8');
...
export function weaponMetric(actor,ground){
  actor.g.updateMatrixWorld(true);let min=Infinity,worst=null;const result={};
  for(const name of ['sword','greatsword','spear']){const node=actor[name];if(!node?.visible)continue;let low=Infinity;
    node.traverse(n=>{if(!n.isMesh)return;for(let i=0;i<n.geometry.attributes.position.count;i++){const p=new Vector3().fromBufferAttribute(n.geometry.attributes.position,i).applyMatrix4(n.matrixWorld),gap=p.y-ground(p.x,p.z);low=Math.min(low,gap);if(gap<min){min=gap;worst={name,vertex:i,point:p.toArray()};}}});result[name]=low;
  }return {min,worst,result};
}

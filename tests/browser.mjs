...
assert.equal(globalThis.__Q_TEST__.interact(), true, 'Ilya starts the archive investigation before the climax');
assert.match(window.document.querySelector('#dialogue-text').textContent, /十二年|決断/);
assert.equal(globalThis.__Q_TEST__.snapshot().characterQuests.ilya, 1);
assert.equal(globalThis.__Q_TEST__.snapshot().npcFlags.ilyaTruth, false);
window.document.querySelector('#dialogue-close').click();
assert.equal(globalThis.__Q_TEST__.defeat('crown_warden'), false, 'the archive investigation cannot be skipped after meeting Ilya');
globalThis.__Q_TEST__.teleport(535, 430);
assert.equal(globalThis.__Q_TEST__.interact(), true, 'the coast archive exposes the erased evacuation record');
assert.match(window.document.querySelector('#dialogue-text').textContent, /評議会|避難路|守印/);
assert.equal(globalThis.__Q_TEST__.snapshot().characterQuests.ilya, 2);
window.document.querySelector('#dialogue-close').click();
globalThis.__Q_TEST__.teleport(0, -615);
...
window.document.querySelector('#pause').click();
window.document.querySelector('#open-journal').click();
assert.match(window.document.querySelector('#journal-content').textContent, /イリヤ.+理解 3 \/ 3/s, 'journal records Ilya archive truth and relationship');
window.document.querySelector('[data-back="paused"]').click();
window.document.querySelector('#resume').click();
snapshot = globalThis.__Q_TEST__.snapshot();
...
assert.match(window.document.querySelector('#ending-copy').textContent, /異なる答え|判断を分け合う/, 'Ilya relationship quest pays off in the ending');

...
window.document.querySelector('#dialogue-close').click();
assert.equal(globalThis.__Q_TEST__.snapshot().quest.step, '人物', 'the world inserts a playable aftermath before the next objective');
globalThis.__Q_TEST__.teleport(-77.2, 238.8);
assert.equal(globalThis.__Q_TEST__.interact(), true, 'Mira appears at the changed village boundary');
...
assert.equal(snapshot.quest.step, '人物');

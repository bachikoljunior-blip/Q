...
test('main wires the production adapter, lifecycle reset, device report and build identity', () => {
  ...
  assert.match(main, /addEventListener\('blur',\(\)=>\{lifecycleGate\.interrupt\(\);clearInput\('blur'\);audio\.suspend\(\)/);
  assert.match(main, /addEventListener\('pagehide',\(\)=>\{lifecycleGate\.interrupt\(\);clearInput\('pagehide'\);save\(\);audio\.suspend\(\)/);
  ...
  assert.match(main, /if\(!await ensureView\(trigger\)\)\{audio\.suspend\(\);return null;\}/);

...
import * as BaseHead from '../src/assets/characters/anatomical-head-data.js';
import * as IdentityHead from '../src/assets/characters/identity-head-data.js';
...
for(const [label,head] of [['base',BaseHead],['Mira',IdentityHead]]){
const {HEAD_ATTRIBUTES:A,HEAD_INDICES:I,HEAD_SOURCE_IDS:V,HEAD_SOURCE_UV_IDS:U,HEAD_SOURCE_TRIANGLE_IDS:F,HEAD_UV_REGIONS:R}=head;
test(label+': every retained surface corner uses its exact original OBJ vertex/UV pair, including same-island seams',()=>{
...
test(label+': LOD preserves each source face orientation without swapping UV corners and the authored cap is nondegenerate',()=>{

...
OUT=ROOT/'docs/evidence/mira-cloth-support-v64'
...
for row,f in enumerate(fixtures):
    both=np.array(f['v63']+f['after']); lo=both.min(0);hi=both.max(0)
    for col,key in enumerate(['v63','after']):
...
fig.suptitle('Rejected v64 candidate: same native death pose, mesh and view\nCPU geometry comparison - NOT a game render; candidate not adopted',fontsize=13)
fig.text(.5,.025,'Red: unchanged chest edge. Blue: flat ground. Left: v63 full 153-node support. Right: v64 51-node curves. No texture or thickness.',ha='center',fontsize=9)

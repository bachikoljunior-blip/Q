...
`npm run compare:combat-priority` は v0.16.0基点と同じ三脅威・同じ入力を比較します。表示対象、画面外分類、操作語の数、表示どおり操作したHP、入力順序によるHP差、ゲーム状態の不変性と48矢のhost上の処理時間を出力します。host時間をスマートフォン実機性能へ読み替えません。

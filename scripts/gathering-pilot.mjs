...
export function playGathering(checkpoint,id){
  ...
  const record=(g,stage)=>{if(!process.argv.includes('--review-saves'))return;const root=new URL('../release/review-saves/',import.meta.url);mkdirSync(root,{recursive:true});writeFileSync(new URL(id+'-'+stage+'.json',root),JSON.stringify(g.serialize())+'\n');};
  for(const choice of d.choices){
    ...
    approach(g.residents.find(n=>n.id===d.actors[0].id));if(choice===d.choices[0])record(g,'ready');assert(gatheringAction(g,id,'start'));reload();approach(d);wait(25);assert(gatheringView(g,id).ready);
    assert(gatheringAction(g,id,'next'));reload();if(choice===d.choices[0])record(g,'middle');

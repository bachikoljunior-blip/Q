...
import {ARROW_LENGTH} from '../src/projectile-shape.js';
...
export function inspectLoadedArrow(turn,quick=false){
 ...
 for(const yaw of(quick?[0]:[0,.9,2.4]))for(const pitch of(quick?[0]:[-.35,0,.35]))for(const timer of(quick?[0]:[.85,.7,.3,.001,0])){
  ...
  a.g.traverse(m=>{if(!m.isMesh)return;geometryCount+=(m.geometry.index?.count??m.geometry.attributes.position.count)/3;meshCount++;if(!m.isSkinnedMesh)return;m.skeleton.update();const vertices=[],names=[];
  ...
  const hits=[],ray=new Ray(),hit=new Vector3();
  function testEdges(edges,targets,reverse=false){for(const edge of edges)for(let i=0;i<3;i++){const from=edge.triangle[i],to=edge.triangle[(i+1)%3],len=from.distanceTo(to);if(len<1e-7)continue;ray.set(from,to.clone().sub(from).normalize());const box=new Box3().setFromPoints([from,to]);for(const target of targets){if(!box.intersectsBox(target.box))continue;if(!ray.intersectTriangle(...target.triangle,false,hit)||hit.distanceTo(from)>len+1e-7)continue;const bones=reverse?edge.bones:target.bones,near=hit.distanceTo(nock);if(near<.025&&bones.every(n=>n==='finger-tip-1-1'||n.startsWith('string-')))continue;hits.push({bones,point:hit.toArray(),fromNock:near});}}}
  testEdges(arrow,blockers);testEdges(blockers,arrow,true);
  const row={yaw,pitch,timer,tailGap:tail.distanceTo(nock),length:tip.distanceTo(tail),gripOffset:point(a.archery.bow,.095).sub(nock).cross(direction).length(),triangles:geometryCount,meshes:meshCount,hits};rows.push(row);
 }

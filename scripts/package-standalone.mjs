...
if(process.argv[1]&&pathToFileURL(process.argv[1]).href===import.meta.url){
  ...
  await writeFile('artifacts/Q-ash-pilgrim-standalone.html',html);
  await writeFile('artifacts/standalone-encoding.json',JSON.stringify(stats,null,2)+'\n');
  console.log(`Created artifacts/Q-ash-pilgrim-standalone.html (${Math.round(Buffer.byteLength(html)/1024)} KiB, self-contained, no network required).`);
}
